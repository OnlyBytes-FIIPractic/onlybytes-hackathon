...
const TreeNode = ({ data, root = false, next = false }) => {
    return (
        <div className="node overflow-hidden me-2">
            <div className='flex flex-row'>
                <div className='flex flex-col me-2'>
                    <div className='vertical-line' hidden={root} />
                    <span style={{ borderRadius: root ? '2rem' : '100%', backgroundImage: `url('img/${data.img1}.webp')`, backgroundSize: 'cover', backgroundPosition: 'center', height: !root ? '40px' : 'auto' , width: !root ? '40px' : 'auto', padding: !root ? '35px' : '20px'  }}>
                        {root ? data.name : <></>}
                    </span>
            ...
            {data.children && (
                <div className="children mx-2">
                    {data.children.map((child, index) => (
                        <TreeNode key={child.id} data={child} next={data.children.length === 1 ? true : index < data.children.length - 1} />
                    ))}
                </div>
            )}
        </div>
